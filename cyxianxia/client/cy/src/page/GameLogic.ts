...
class GameLogic {
    ...
    private seachWordWithArr(str: string, indexArr: Array<number>): string {
        let allWords = StaticVar.idiomJson;// GameData.getInstance().getAllWords();
        let min = indexArr[0];
        let max = indexArr[0];
        ...
        let num = StaticVar.getRandomIntFull(0, wordArr.length - 1);
    ...
    public startSetWord(data: any, length: number, arr: Array<Array<any>>): any {
        let allWords = StaticVar.idiomJson;//GameData.getInstance().getAllWords();
        //随机取一个成语
        let firstNum = StaticVar.getRandomIntFull(0, allWords.length - 1);
