...
class Game extends GameUI {
	...
	private idiomCon: egret.Sprite;
	private idiomArr: any;
	private completeArr: any;
	public show(data?): void {
		...
		egret.setTimeout(() => {
			console.log('sss', this.idiomArr);
		}, this, 2000)
	}


	private getArr(idiomArr, arr, str) {
		wy.LoadingData.close();
		console.log('arr====', '\n', idiomArr, '\n', '\n', arr, '\n', str, '\n');

		for (let k = 0; k < idiomArr.length; k++) {
			let obj = idiomArr[k];
		...
		for (let i = 0; i < 9; i++) {
			...
			for (let j = 0; j < 9; j++) {
				...
				let item: GameItem = new GameItem();
		...
		for (let i = 0; i < 9; i++) {
			for (let j = 0; j < 9; j++) {
				...
				if (str_row.length == 4) {
					...
					for (let _a = 0; _a < this.idiomArr.length; _a++) {
	...
	private failMethod(arr_row) {
		for (let _b = 0; _b < arr_row.length; _b++) {
			for (let i = 0; i < 9; i++) {
				for (let j = 0; j < 9; j++) {
					let obj = this.totalArr[j][i];
	...
	/**创建下面的单词 */
	private createBootomItem(itemBottomArr) {
		for (var i = 0; i < itemBottomArr.length; i++) {
			let bottomItem = itemBottomArr[i];
			bottomItem.x = 70 + 90 * (i % 7);
			let _yNum = Math.floor(i / 7) % 7;
