module wy {
	...
	export class Data {
		 ...
		 * @version 0.0.3
	 	 * @platform egret3.0.3
		 */
		public static mustopenid: boolean = false;
		/**
		 * 舞台宽
		 * 
		 * @version 0.0.3
		...
		public static init(): void {
			...
			this.stageW = wy.GameInterface.stage.stageWidth;
			this.stageH = wy.GameInterface.stage.stageHeight;

			this.openid = this.getItemByName('openid');
			this.headimgurl = this.getItemByName('headimgurl');
			this.nickname = this.getItemByName('nickname');
		...
		public static getItemByName(item: string): any {
			var ans = this.getCookie(item);
			if (ans) {
				ans = decodeURIComponent(decodeURIComponent(ans));
				if (!ans) {
